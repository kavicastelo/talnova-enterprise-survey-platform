# FEAT-005 Process-Flow Audit Record: PF-005-01

| Metadata Field | Value |
|---|---|
| **Process Flow ID** | `PF-005-01` |
| **Process Flow Title** | Multi-Channel Survey Campaign Creation & Launch Wizard |
| **Feature Suite** | `FEAT-005: Multi-Channel Survey Distribution Engine` |
| **Target Role(s)** | `PROJECT_ADMIN`, `HR_MANAGER` |
| **Primary Route** | `/distribution` |
| **API Endpoints** | `POST /api/v1/campaigns`, `POST /api/v1/campaigns/ai/predict-optimal-time` |
| **Audit Status** | **PASS** |
| **Audit Date** | 2026-08-11 |
| **Auditor** | Lead Frontend Architect |

---

## 1. Process Flow Specification & Requirements

### 1.1 Objective
Enable HR Managers and Project Administrators to launch multi-channel survey campaigns across Email, SMS, QR Codes, Kiosk PINs, MS Teams, and Slack targeting specific Organization Node sub-trees (`FR-DST-001`, `FR-DST-007`).

### 1.2 Authoritative Rules & Guards Enforced
- **FR-DST-001**: 6 Core Distribution Channels supported (`EMAIL`, `SMS`, `QR_CODE`, `KIOSK_PIN`, `TEAMS`, `SLACK`).
- **FR-DST-002**: Multi-Tier Cryptographic Token Generation configuration (`SEMI_ANONYMOUS`, `AUTHENTICATED`, `FULLY_ANONYMOUS`, `KIOSK`).
- **FR-DST-007**: Audience Target Filtering via Organization Node tree scope selection.
- **VR-DST-001**: `campaignId` regex pattern `^CMP-[A-Za-z0-9_-]{3,20}$`.
- **VR-DST-003**: Campaign expiration date must be at least 24 hours in the future and maximum 90 days.
- **VR-DST-004**: At least one distribution channel required.

---

## 2. Technical Implementation Architecture

### 2.1 Component Mapping
- **Studio Page**: [DistributionStudioPage.tsx](file:///d:/talnova/talnova-enterprise-survey-platform/frontend/src/features/distribution/pages/DistributionStudioPage.tsx)
- **Wizard Component**: [CampaignLaunchWizard.tsx](file:///d:/talnova/talnova-enterprise-survey-platform/frontend/src/components/distribution/CampaignLaunchWizard.tsx)
- **API Service Layer**: [distributionApi.ts](file:///d:/talnova/talnova-enterprise-survey-platform/frontend/src/features/distribution/api/distributionApi.ts) (`createCampaign`, `predictOptimalTime`)
- **React Query Hooks**: [useDistributionQueries.ts](file:///d:/talnova/talnova-enterprise-survey-platform/frontend/src/features/distribution/api/useDistributionQueries.ts) (`useCreateCampaignMutation`, `usePredictOptimalTimeMutation`)

### 2.2 Sequence Flow
```mermaid
sequenceDiagram
    autonumber
    actor HR as HR Manager / Project Admin
    participant UI as CampaignLaunchWizard
    participant API as distributionApi
    participant GW as API Gateway (:8080)
    participant SDS as survey-distribution-service (:8085)

    HR->>UI: Select "Launch New Campaign Wizard"
    HR->>UI: Step 1: Set Title & Anonymity Tier ("SEMI_ANONYMOUS")
    HR->>UI: Step 2: Select Org Nodes ("N-301", "N-302")
    HR->>UI: Step 3: Toggle Channels ("EMAIL", "TEAMS") & set Expiration Date
    HR->>UI: Click "Recommend Hour"
    UI->>API: predictOptimalTime(projectId, channel, recipientCount)
    API->>GW: POST /api/v1/distribution/ai-optimal-time
    GW-->>UI: Return recommended UTC hour (e.g. 09:00 UTC)
    HR->>UI: Step 4: Click "🚀 Launch Campaign Now"
    UI->>API: createCampaign(payload)
    API->>GW: POST /api/v1/campaigns
    GW->>SDS: Store campaign & publish token generation jobs to Kafka
    SDS-->>UI: Return 201 Created (CampaignResponseDTO)
    UI->>HR: Show Toast "Survey campaign CMP-XXXX launched!" & switch to Live Monitor
```

---

## 3. UI/UX Verification & States Matrix

| State Category | Expected Visual / Behavioral Requirement | Implementation Verification | Status |
|---|---|---|---|
| **Progressive Wizard** | 4-step progressive wizard with back/next navigation. | Verified in `CampaignLaunchWizard.tsx` lines 123-324. | **PASS** |
| **Org Scope Estimator** | Calculates and renders total recipient headcount dynamically based on selected node checkboxes. | Verified in `CampaignLaunchWizard.tsx` lines 52-55 & 178. | **PASS** |
| **Expiration Date Validation** | Rejects expiration dates $< 24\text{ hours}$ in future (`VR-DST-003`). | Verified in `CampaignLaunchWizard.tsx` line 251. | **PASS** |
| **Channel Guard** | Prevents advancing if 0 channels selected (`VR-DST-004`). | Verified in `CampaignLaunchWizard.tsx` line 239. | **PASS** |
| **AI Dispatch Predictor** | Displays recommended UTC dispatch hour and open-rate improvement % alert. | Verified in `CampaignLaunchWizard.tsx` lines 256-276. | **PASS** |

---

## 4. Test & Verification Evidence

- **TypeScript Compilation**: `npm run build` (`tsc && vite build`) passed with **0 errors**.
- **Unit & Domain Tests**: `npm run test` passed **14/14 test suites (64/64 tests passed)**.
  - `src/__tests__/surveyDistributionDomain.test.ts`: **PASS**

---

## 5. Certification Sign-off

- [x] Process Flow **PF-005-01** specification fully implemented.
- [x] Multi-channel campaign wizard and API Gateway integration verified.
- [x] Audit Status: **PASS**.
